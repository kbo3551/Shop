...
<div class="container-fluid">
	...
	<div class="container-fluid" style="text-align: center">
		...
		<div style="margin: auto; text-align: center;">
			<a class="btn btn-info" href="<%=request.getContextPath()%>/admin/adminIndex.jsp?currentPage=1">처음으로</a>
		<%
			if(currentPage != 1){
		%>
				<a class="btn btn-info" href="<%=request.getContextPath()%>/admin/adminIndex.jsp?currentPage=<%=currentPage-1%>">이전</a>
		<%
			}
			...
			for(int i=startPage; i<=endPage; i++) {
				if(endPage<=lastPage){
		%>
					<a class="btn btn-info" href="<%=request.getContextPath()%>/admin/adminIndex.jsp?currentPage=<%=i%>"><%=i%> </a>
		<%
		...
		<%
			...
			%>
				<a class="btn btn-info" href="<%=request.getContextPath()%>/admin/adminIndex.jsp?currentPage=<%=currentPage+1%>">다음</a>
			<%
			}
			%>
			<a class="btn btn-info" href="<%=request.getContextPath()%>/admin/adminIndex.jsp?currentPage=<%=lastPage%>">끝으로</a>
		</div>
	<%
		}
	%>
